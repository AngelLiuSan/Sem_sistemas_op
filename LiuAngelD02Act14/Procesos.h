...
class proceso {
    ...
    public:
        ...
        void imprimirP() {//cambio
            ...
            cout << "\n\tTR: " << gettr() << endl << endl;
            cout<<"\n\t Tam: "<<tam<<endl;
            cout << "\n\tQuantum: " << quantum << endl << endl;//agregado
            }
            ...
            if (kbhit()) {
                ...
                switch(tecla) {
                    case 'p':
                        ...
                        imprimirT();
                        if(!nuevos.empty()){
                        cout<<"\n\tProcesos nuevos: "<<nuevos.size()<<" | Siguiente: id:"<<nuevos.front()->getID()<<" tam:"<<nuevos.front()->tam<<endl;
                        }else{
                            cout<<"\n\tProcesos nuevos: 0"<<endl;
                        }
                        cout<<"\n\tReloj global: "<<reloj<<endl;
                        cout<<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
                        while(tecla!='c') {
        ...
        void imprimir() {//agregado lo de memoria
            tr = gettiempo()-tt;
            while(tr > 0) {
            if(quantum == QU){
                if(listos.size()>0){
