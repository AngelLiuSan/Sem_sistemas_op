...
long long proceso::ultimoID;
long long  proceso::reloj;
...
bool archivoContieneTexto(const std::string& nombreArchivo)
{
...
////////////////////////////////////////////////////////////////
    ...
    cout<<"suspendidos.txt";
    do{
        while (!proceso::nuevos.empty() || !proceso::bloqueados.empty() || (!proceso::listos.empty() && proceso::listos.front()->getID() != -1) || archivoContieneTexto("suspendidos.txt")){
            ...
            if(proceso::listos.size()>0){
                ...
                proce->imprimirVP();}
            if (proceso::nuevos.empty() && proceso::bloqueados.empty() && proceso::listos.empty() && (proceso::listos.empty() || (proceso::listos.front() && proceso::listos.front()->getID() == -1)) &&
                    (archivoExiste("suspendidos.txt")== false || archivoContieneTexto ("suspendidos.txt")==false )){
                proce->imprimirVP();
    ...
    while (archivoContieneTexto("suspendidos.txt"));
    proce->imprimirVP();
    imprimirF(proceso::terminados);
    system("pause");
